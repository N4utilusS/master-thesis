...
\chapter{Solution}
	...
	The final implementation is built on 2 layers. The upper layer is the state machine, having for each state a specific sub-behaviour in the lower layer. Figure \ref{state_machine} illustrates the whole structure of the upper layer. Only part of the states rely on virtual physics.
...
%		  				(NORMAL)		edge		node		{\color{gray} 2}				(OBS)
%		  				(NORMAL)		edge		node		{\color{gray} 3}				(HU)
%		  				(NORMAL)		edge		node		{\color{gray} 4}				(DEF)
