...
\begin{figure}
	...
	%Mauro, Gaëtan, Giovanni, Anthony, Brian, Family and friends.
	\epuck{-5}{15}
	\epuck{-5}{12}
	\epuck{-5}{9}
	\epuck{-5}{6}
	...
	\draw (-2.9,15) node{Mauro Birattari};
	\draw (-2.8,12) node{Gaëtan Podevijn};
	\draw (-2.3,9) node{Andreagiovanni Reina};
	\draw (-2.8,6) node{Anthony Antoun};
	\draw (-2.9,3) node{Brian Delhaisse};
...
\end{comment}
	...
	\section{Flocking and Pattern Formation with a Guide}
	
		[Work related to what I do. Conclude on why it cannot be applied to my problem. Make connections with this part later in the document.]
...
[Explain the solution independently of the tools used to implement the solution. Make the reader understand that this problem is new.
	...
	[The problems I will have to solve?]
...
%////////////////////////////////////////////////////////////
	...
	\section{The Software}
		...
		\subsection{The Global Behaviour (state machine)}
...
%			\end{enumerate}
	...
	\end{figure}
		...
		\subsection{The Pattern Formation}
	...
	$$\vec{f} = -\vec{\nabla}P$$, P being the system's potential. The following sections will explain in detail the different potentials that were implemented to obtain the desired behaviour.
		...
		\subsubsection{Potentials (title needed?)}
		...
		In this section, the potentials are grouped by states in which they are used. Since virtual physics are used in only 2 states, we have only 2 groups.
			...
			The controller enters the \emph{Human} state at the beginning of the time step only if a human is found nearby. The complete potential for this state is the sum of 3 components: the \emph{human potential}, the \emph{gravity potential} and the \emph{agent repulsion potential}.
				...
				\subparagraph{Gravity Potential}
				
				[Description of the evolution of the gravity potential.]
				
				\subparagraph{Agent Repulsion Potential}
	...
	[I will in this section describe the need for a device to detect a human and its development. What are the objectives of the hardware? The choices we made to get the final solution. How we built it. Calibration an adaptation process to make the current algorithm compatible with the new hardware.]
...
\chapter{Experiments}
	...
	[The measurements and the tests on the final behaviour.]
		...
		[Metrics I will use. Their description. How I will compute them.]
			...
			\subsubsection{Correct Distance}
			
			[Do the robots respect the correct wanted distance?]
			
			\subsubsection{Robot Density}
...
\chapter{Conclusion}
	...
	[The future works that would be interesting from my point of view.]
		...
		\subsection{Human Guidance}
			\subsubsection{Zero Visibility Areas or Blind People}
			\subsubsection{Human Motion Synchronisation}
			\subsubsection{Vehicle Guidance}
