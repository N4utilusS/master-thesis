...
In this chapter, we introduce the different experiments we conducted with the real robots. The tests were made with the E-puck robotic platform \citep{mondada2009puck}. We can classify the tests into two sets: one assessing the system quantitatively, and one assessing the system qualitatively (also know as the demonstration). We first describe the quantitative assessments, and their results in the section \ref{sec:characterisation_system}. Then we switch to the other kind of tests in the section \ref{sec:demonstration}.
	...
	In this section, we assess the performance of our solution without any human controlling the swarm. The solution is evaluated quantitatively with several criteria.
		...
		\subsection{The Shoe Detection Range}
			...
			In this section, we determine the range of detection of the shoes by the robots. We determine the maximum distance at which the robots see the shoes. Figure \ref{fig:shoe_detection_range} illustrates the experiment we conducted to determine this distance. The maximal distance goes from 59 cm in front and behind the shoes, to 65 cm on the right and 70 cm on the left. This distance is acceptable given the size of the robots. It gives enough room to the user.
		...
		\subsection{Circle Properties}
			...
			\subsubsection{Set-up}
				...
				In this section, we explain how we conducted the experiments assessing our solution. These experiments took place in the IRIDIA laboratory, in the arena room. The arena room is the place were the academic staff can manipulate the robots. We used 8 e-pucks for every experiment. They were all equipped with tags to allow the arena tracking system \citep{stranieri2013iridia} to locate them, and transmit the information to the simulator. At the beginning of each experiment, the robots were placed according to the type of experiment undertaken. There are two types of experiment. The first one consists in a random starting position for all the robots. Figure \ref{fig:random_setup} illustrates the first set-up type. The other type consists in a specific starting position: all the robots are placed on the right of the shoes. Each experiment lasted 3 minutes. The arena is a square, bounded with wood boards or walls (on the right of the picture on the two figures).
			
				\begin{figure}[!htp]
					\includegraphics[width=\textwidth]{images/setup_random.png}
					\caption{The random setup}{}
...
@techreport{stranieri2013iridia,
  ...
  author={Stranieri, A and Turgut, A and Francesca, G and Reina, A and Dorigo, M and Birattari, M},
  year={2013},
  institution={Technical report TR/IRIDIA/2013-013, IRIDIA, Universit{\'e} Libre de Bruxelles, Brussels, Belgium}
}

@article{reinaaugmented,
  ...
  journal={Adaptive Hardware and Systems (AHS)}
}
