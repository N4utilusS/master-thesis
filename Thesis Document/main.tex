...
%	\end{figure}
	...
	\begin{figure}\centering
		...
		\caption{Unknown environment in which a human must evolve while avoiding invisible dangerous areas. This thesis explores a solution to guarantee safeness in such circumstances. Applications for this type of problems already exist: mine fields, radioactive areas, etc. This figure represents a path to follow from A to B in a dangerous zone.}
	...
	The solution proposed involves the use of a swarm of robots. Swarm robotics seems fit to this kind of application, since it is compatible with unknown environments thanks to its flexible, robust and scalable characteristics \citep{brambilla2013swarm}. In case of failure of one of a few robots, the system would continue to provide sufficient performance thanks to its scalability and robustness.\\
	
	The swarm ideally forms a round shield around the user to ensure a 360° protection. All the robots try to stay at the same distance from each other and the person. To achieve this, the final solution relies on the pattern formation theory widely used in swarm robotics. If the amount of robots is not high enough to form a complete circle, an arc is formed at the front to always shield the most critical zone.
	...
	\begin{figure}\centering
		...
		\caption{Representation of a human (H) helped by a swarm. The swarm tells the human that a dangerous zone is located north-east by visual communication. The swarm stays at the boundary to form a \enquote{shield}. The direction taken by each individual in the swarm is given by the triangle inside (here heading north).}
