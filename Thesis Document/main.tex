...
\chapter{Solution}
	...
	$$\frac{\mbox{Surface}}{\mbox{Perimeter}} = \frac{r}{2}~\mbox{, where r is the ray of the circle.}$$
	...
	Indeed, the laws of physics force the system to get to a state of minimum energy, i.e., to reach a global minimum of the potential function of the system. Since the force is proportional to the derivative of the corresponding potential, the minimum of the potential function means the disappearance of the forces.
	
	$$\vec{f} = -\vec{grad}~P$$, P being the system's potential. The following sections will explain in detail the different potentials that were implemented to obtain the desired behaviour.
	
		\subsection{Potentials}
		
		
			\subsubsection{Human Potential}
			\subsubsection{Gravity Potential}
			\subsubsection{Agent Repulsion Potential}
			\subsubsection{Default Potential}
